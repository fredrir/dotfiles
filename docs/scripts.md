...
`dmux`, the wezterm-mux + tmux session manager, was extracted to its own
repository: https://github.com/fredrir/dmux, and is no longer built, deployed
or documented here. Its CLI reference — verbs, ref grammar, backend-instance
states, the `--format json` envelope — lives in that repo.

Nothing in this repository installs dmux any more: the crate, the mux service
units, the LaunchAgents, the desktop entry point and the shell hooks all went
with it. What remains is the GUI-side signed bridge under
`shared/wezterm/wez/dmux_bridge/`, which is inert unless `DMUX_WEZ_FIRST=1`
and is kept because it is wezterm configuration rather than dmux itself.
