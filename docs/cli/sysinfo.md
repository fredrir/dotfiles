...
| `--tier <TEXT>`         | Selects the quick, standard, or heavy benchmark tier.           |
| `--only <TEXT>`         | Limits a run to a comma-separated list of measurement families. |
| `--note <TEXT>`         | Records why a benchmark run was taken.                          |
| `--tag <TEXT>`          | Adds a label to a benchmark run.                                |
| `--host <TEXT>`         | Selects the host to record, list, assess, or prune.             |
| `--workdir <TEXT>`      | Selects the directory used by the disk benchmark tier.          |
...
| `--limit <INTEGER>`     | Limits the number of stored runs listed.                        |
| `--all`                 | Includes noisy and aborted runs in a listing.                   |
| `--keep <INTEGER>`      | Sets the number of runs retained per machine configuration.     |
