...
### Wave 3b — dispatched 2026-08-23, base `88b0e4a` (every WS-A item, WS-C, WS-E.1/E.3 rows 7–8/10/13/14, B.3, B.5, F.1 landed; gate 1055/0/1)

| agent | items | worktree / branch | owned paths |
| --- | --- | --- | --- |
| B | WS-B.1 (`StaleIncarnation` arm + every consumer), B.2 (C/D split, remedy texts), B.4 (doctor epoch probe, A–F, JSON; reports a set `DMUX_RUNTIME_DIR`), Dead's two `ls` tests, A.12's orphaned doc, `peer_scan_error_code` mapping, `frozen_local_target` unification | `~/packages/dmux-p11/b`, `p11/b` | `backend/scope.rs`, `ls_cli.rs`, `doctor.rs`, `gui_lifecycle.rs`, `tests/ls_cli.rs`, new tests; line-scoped arm in the eight consumer files |
| D3 | WS-D.3 (one resolver; truth table vouches for production; case 44 once), E.3 row 12, O's dev/ino reader at `connect_cli.rs`, `BindingRow.server_epoch` | `d3`, `p11/d3` | `resolve.rs`, the five resolution sites, `connect_cli.rs`, `inventory.rs` literal, `registry/mod.rs` BindingRow lines, `tests/resolver_truth_table.rs`, `tests/connect_cli.rs` |
| T | WS-A.8 tmux half, A.9 adapter readers, the `tmux_bootstrap` `AlreadySet` refusal policy, E.2 tmux repros, E.3 row 9 (trait removals, both impls, the double) | `t`, `p11/t` (pending target) | `backend/tmux.rs`, `tests/provider_tmux.rs`; row 9 grants on `backend/mod.rs`, `backend/wez.rs`, `tests/provider_contract.rs`, the operations test fake |
| QA | WS-E.2 shell/Lua sites, E.4 gated-dispatch coverage, flake triage (`json_envelope` split-ls document, `capability_gate` probe timeouts) | `qa`, `p11/qa` (pending) | `tests/cases/**`, the two zsh files and `controller.lua`, the dispatch harnesses, new `tests/gated_dispatch.rs` |
| D | WS-D.1 `repair rebind`, WS-D.2 adoption journal source token (schema appendix + lossless migration) | `d`, `p11/d` (pending) | `space_cli::RepairCmd` + rebind handler, `registry/{mod,schema,reconcile}.rs`, `tests/registry/**`, new `tests/repair_rebind.rs` |
| GUI | WS-E.3 rows 11 and 15 (five `gui.rs` helpers; `select_compatible_domain`'s identity check adopted; rotation success-path test), stale texts | `gui`, `p11/gui` (pending) | `gui.rs`, `gui_cli.rs` rows 11/15 sites |
