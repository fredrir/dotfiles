...
| T | WS-A.8 tmux half, A.9 adapter readers, the `tmux_bootstrap` `AlreadySet` refusal policy, E.2 tmux repros, E.3 row 9 (trait removals, both impls, the double) | `t`, `p11/t` (base `9634c72`) | `backend/tmux.rs`, `tests/provider_tmux.rs`; row 9 grants on `backend/mod.rs`, `backend/wez.rs`, `tests/provider_contract.rs`, the operations test fake |
| QA | WS-E.2 shell/Lua sites, E.4 gated-dispatch coverage, flake triage (`json_envelope` split-ls document, `capability_gate` probe timeouts) | `qa`, `p11/qa` (base `9634c72`) | `tests/cases/**`, the two zsh files and `controller.lua`, the dispatch harnesses, new `tests/gated_dispatch.rs` |
| D | WS-D.1 `repair rebind`, WS-D.2 adoption journal source token (schema appendix + lossless migration) | `d`, `p11/d` (base `9634c72`) | `space_cli::RepairCmd` + rebind handler, `registry/{mod,schema,reconcile}.rs`, `tests/registry/**`, new `tests/repair_rebind.rs` |
