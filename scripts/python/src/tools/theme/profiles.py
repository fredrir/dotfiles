...
SELECTION_FILE = os.path.join(ROOT, "config/profiles.dotfile")
...
def _fail(problems):
    ...
    raise SystemExit(f"dotfile theme: config/profiles.dotfile is not usable:\n{listed}")
...
def read_selection(owned):
    try:
        ...
        raise SystemExit(f"dotfile theme: config/profiles.dotfile line {error.number}: {reason}")
...
def default_profile():
    ...
    for entry in entries:
        ...
        if key == THEME_KEY and value:
            return value
    raise SystemExit(
        f"dotfile theme: config/profiles.dotfile has no '{THEME_KEY}' under '{DEFAULT_GROUP}'"
    )
...
def _lines():
    try:
        ...
        raise SystemExit(
            "dotfile theme: config/profiles.dotfile is missing, cannot tell which profiles to use"
        )
