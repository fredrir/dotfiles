...
def keys_file(ctx):
    return os.path.join(ctx.root, "config/keys.dotfile")
...
def load_recipients(ctx):
    ...
    try:
        ...
        die(blocks.describe(error, "config/keys.dotfile", "'recipients {' block"))
        return found
    for entry in entries:
        if entry.opens:
            if entry.block != "recipients":
                die(f"config/keys.dotfile:{entry.number}: unknown block '{entry.block}'")
            continue
        if "=" not in entry.text:
            die(f"config/keys.dotfile:{entry.number}: expected <label> = <age public key>")
        label, key = entry.split("=")
        if not LABEL.match(label):
            die(f"config/keys.dotfile:{entry.number}: bad label '{label}'")
        if not AGE_KEY.match(key):
            die(f"config/keys.dotfile:{entry.number}: '{label}' is not an age public key")
        if label in found:
            die(f"config/keys.dotfile:{entry.number}: duplicate label '{label}'")
        found[label] = key
    return found
