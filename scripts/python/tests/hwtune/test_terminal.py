...
def history(tmp_path):
    ...
    directory = tmp_path / "benchmarks" / "hosts" / run["host"] / "runs"
    directory.mkdir(parents=True)
    (directory / f"{run['run_id']}.json").write_text(json.dumps(run))
    (tmp_path / "benchmarks" / "store.json").write_text(json.dumps({"schema": 1}))
    return dict(
        os.environ,
        HOME=str(tmp_path),
        DOTFILE_ROOT=str(tmp_path),
        HWTUNE_BENCHMARKS=str(tmp_path / "benchmarks"),
        HWTUNE_HOST=run["host"],
        TERM="xterm-256color",
        NO_COLOR="1",
        PATH="/usr/bin:/bin",
    )
...
class Terminal:
    def __init__(self, environment):
        binary = rust_binary("hwtune", "hwtune")
...
def test_native_history_menu_restores_terminal_after_resize_and_cancel(history, cancel):
    history_root = Path(history["HWTUNE_BENCHMARKS"])
    ...
    try:
        terminal.expect("hwtune bench")
        terminal.expect(history["HWTUNE_HOST"])
