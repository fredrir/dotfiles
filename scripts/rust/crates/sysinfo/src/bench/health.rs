...
use crate::model::{HealthIssue, Severity};
...
pub fn regression_issue(delta: &Delta, baseline: &Run, latest: &Run) -> HealthIssue {
    ...
    HealthIssue {
        ...
        action: format!(
            "Re-run sysinfo bench run --only {} to confirm, then look for thermal or configuration causes",
    ...
    if let Ok(started) = DateTime::parse_from_rfc3339(&latest.started) {
        ...
        if age >= 120 {
            issues.push(HealthIssue {
                ...
                action: "Run sysinfo bench run to refresh the series".into(),
            });
        }
    }
