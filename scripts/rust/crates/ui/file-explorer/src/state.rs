...
impl<L: Clone + Eq> State<L> {
    ...
    pub(crate) fn edit_prompt(
        ...
        let mut text = ui_widgets::PromptBuffer::new(&prompt.text);
        match edit {
            PromptEdit::Insert(character) => text.insert(character),
            PromptEdit::Backspace => {
                text.backspace();
            }
            PromptEdit::Kill => text.clear(),
            PromptEdit::WordBack => text.word_back(),
        }
        let kind = classify(text.as_str());
        self.set_prompt(text.into_string(), kind);
    }
    ...
    pub(crate) fn move_by(&mut self, amount: isize) {
        ...
        viewport.move_by(amount, self.rows.len(), ui_widgets::Navigation::Clamp);
    ...
    pub(crate) fn settle(&mut self, viewport_rows: usize) -> bool {
        ...
        viewport.settle(self.rows.len(), viewport_rows);
        self.cursor = viewport.cursor;
        self.offset = viewport.offset;
    ...
    fn rebuild_rows(&mut self) {
        ...
        match &self.prompt {
            Some(Prompt {
                ...
                self.search.filter_into(text, &mut self.rows);
            }
