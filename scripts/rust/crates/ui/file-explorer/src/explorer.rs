...
{
    pub(crate) source: S,
    pub(crate) start: S::Location,
    pub(crate) style: &'a Style,
    pub(crate) view: V,
    pub(crate) config: ExplorerConfig,
    pub(crate) initial_focus: Option<S::Location>,
}
    ...
    S: FileSource,
{
    ...
    pub fn into_async(self) -> crate::AsyncExplorer<'a, S, V> {
        crate::AsyncExplorer::new(self)
    }
        ...
        T: Terminal,
    {
        ...
        loop {
            ...
            if dirty {
                ...
                let mut frame = self.frame(state, terminal, help, style.style());
                if state.settle(frame.viewport_rows) {
                    frame = self.frame(state, terminal, help, style.style());
                }
