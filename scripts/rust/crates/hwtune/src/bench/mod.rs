    ...
    clap::Command::new("bench")
        ...
        .subcommand(
            measurements("run", "Measure this machine and store the result")
                ...
                .arg(option("stability", "Link a stability session").action(ArgAction::Append))
        ...
        .subcommand(experiments::command())
        .subcommand(
...
pub fn run(args: &ArgMatches) -> Result<(), String> {
    ...
    let Some((name, args)) = args.subcommand() else {
        if !io::stdin().is_terminal() || !io::stdout().is_terminal() {
            workstation::cli::decorate(command().bin_name("hwtune bench"))
                .print_help()
                .map_err(|e| e.to_string())?;
            println!();
            return Ok(());
        }
        return menu::open(&store, "", nonempty(text(args, "host")));
    };
    match name {
        "run" => measure(&store, args),
        "plan" => plan::run(args),
        "report" => experiments::run(&store, args),
        "show" => {
            if text(args, "target").is_empty() {
                return menu::open(&store, "show", nonempty(text(args, "host")));
            }
        ...
        "compare" => {
            ...
            if left.is_empty() || right.is_empty() {
                return menu::open(&store, "compare", nonempty(text(args, "host")));
            }
            emit_comparison(
        ...
        "trend" => {
            if text(args, "target").is_empty() || text(args, "metric").is_empty() {
                return menu::open(&store, "trend", nonempty(text(args, "host")));
            }
        ...
        "baseline" => {
            match text(args, "action") {
                ...
                "set" => {
                    if text(args, "target").is_empty() {
                        return menu::open(&store, "baseline", nonempty(text(args, "host")));
                    }
...
fn resolve_host(explicit: &str) -> Result<(String, bool), String> {
    let known = hosts::load_hosts()?;
    let name = crate::paths::host_name(nonempty(explicit))?;
    ...
    let detected = sysinfo::inventory::local_hostnames();
    let primary = detected.first().map(String::as_str).unwrap_or("unknown");
    eprintln!(
        "hwtune bench: this machine is not described in config/hosts.dotfile (hostname {primary})"
    );
    ...
    match choice {
        Some(0) => {
            ...
            if !sysinfo::inventory::valid_name(&name) {
            ...
            let host = sysinfo::inventory::Host {
                name: name.clone(),
                hostnames: detected,
                role,
                ..Default::default()
            };
            let path = hosts::append_host(&host)?;
            hosts::save_host(&name)?;
            println!(
                "hwtune bench: wrote {name} to {} and pinned this machine to it",
                path.display()
            );
            Ok((name, true))
        }
...
fn measure(store: &Store, args: &ArgMatches) -> Result<(), String> {
    ...
    let (host, persist) = if args.get_flag("no-save") {
        (
            crate::paths::host_name(nonempty(text(args, "host")))?,
            false,
        )
    } else {
        resolve_host(text(args, "host"))?
    };
    ...
    .map_err(|e| e.to_string())?;
    let stability = args
        .get_many::<String>("stability")
        .into_iter()
        .flatten()
        .cloned()
        .collect::<Vec<_>>();
    let context = Some(provenance::capture(&host, &stability)?);
    ...
    let measured;
    {
        ...
        if persist {
            let _history_lock = store.exclusive()?;
    ...
    if persist
        && let Some(reference) = store.baseline_run(&measured.host, &measured.epoch())?
