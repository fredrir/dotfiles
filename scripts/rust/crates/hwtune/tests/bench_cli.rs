...
fn binary(store: &Store) -> Bin {
    Bin::new(env!("CARGO_BIN_EXE_hwtune"))
        .arg("bench")
        .env("HWTUNE_BENCHMARKS", &store.root)
        .env(
            "HWTUNE_MEASUREMENT_LOCK",
            store.root.with_extension("measurement.lock"),
        )
        .plain()
}
#[test]
...
fn fixture_measurement(root: &std::path::Path, store: &Store, family: &str) -> Bin {
    fixture_measurement_for_host(root, store, family, "archie")
}

fn fixture_measurement_for_host(
    root: &std::path::Path,
    store: &Store,
    family: &str,
    host: &str,
    ...
    let config = root.join("config/hosts.dotfile");
...
#[test]
fn unsaved_unknown_hosts_measure_without_adoption_or_host_mutations() {
    let root = tempfile::tempdir().unwrap();
    let store = Store::new(root.path().join("history"));
    let config_home = root.path().join("config-home");
    let command = fixture_measurement_for_host(root.path(), &store, "cache", "unregistered");
    sysbench_fixture(root.path());
    let hosts_path = root.path().join("config/hosts.dotfile");
    let before = fs::read(&hosts_path).unwrap();
    let result = command
        .arg("--no-save")
    ...
    assert!(result.success(), "{result:?}");
    let run: Run = serde_json::from_str(&result.stdout).unwrap();
    assert_eq!(run.host, "unregistered");
    assert!(!run.metrics.is_empty());
    assert!(!store.root.exists());
    assert_eq!(fs::read(hosts_path).unwrap(), before);
    assert!(!config_home.join("dotfile/host").exists());
}

#[test]
fn global_host_is_forwarded_into_benchmark_arguments() {
    let root = tempfile::tempdir().unwrap();
    let store = Store::new(root.path().join("history"));
    let saved = run(1);
    store.save_run(&saved).unwrap();
    let mut other = run(2);
    other.host = "macie".into();
    store.save_run(&other).unwrap();
    let result = Bin::new(env!("CARGO_BIN_EXE_hwtune"))
        .args(["--host", "archie", "bench", "list"])
        .env("HWTUNE_BENCHMARKS", &store.root)
        .plain()
        .run();
    assert!(result.success(), "{result:?}");
    assert!(result.stdout.contains("archie"), "{result:?}");
    assert!(!result.stdout.contains("macie"), "{result:?}");
}
