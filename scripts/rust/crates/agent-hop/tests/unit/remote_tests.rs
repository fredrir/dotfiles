...
fn preflight_checks_the_workspace_agent_and_login_shell() {
    ...
    assert!(script.starts_with(
        "export PATH=\"${DOTFILES_COMPILED:-$HOME/dotfiles/.bin}:$HOME/.local/bin:$PATH\"; "
    ));
    assert!(script.contains("test -d '/home/fred rir/project'"));
...
fn machine_requests_have_no_tty_and_cannot_prompt_or_inject_shell_words() {
    ...
    assert_eq!(
        script,
        "export PATH=\"${DOTFILES_COMPILED:-$HOME/dotfiles/.bin}:$HOME/.local/bin:$PATH\"; exec agent-hop '__machine' 'preview' '--session' 'id'\\''; touch /tmp/nope; '\\'''"
    );
