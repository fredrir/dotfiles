...
pub fn read_manifest(path: &Path) -> Result<Vec<String>, String> {
    ...
    text.lines()
        ...
        .filter_map(|(offset, line)| {
            ...
            (!group.is_empty()).then_some((offset, group))
        })
        .map(|(offset, group)| {
            validate_relative(group)
                .map_err(|e| format!("{}:{}: {e}", path.display(), offset + 1))?;
            Ok(group.to_string())
        })
        .collect()
}

pub fn load_overrides(path: &Path) -> Result<BTreeMap<String, String>, String> {
...
pub fn load_targets(context: &Context) -> Result<BTreeMap<String, PathBuf>, String> {
    ...
    let family = platform_family(context)?;
    ...
    for (offset, raw) in text.lines().enumerate() {
        ...
        validate_relative(path)
            .map_err(|e| format!("{}:{}: {e}", context.targets_file.display(), offset + 1))?;
...
fn platform_family(context: &Context) -> Result<&'static str, String> {
    if let Some(forced) = context.env("DOTFILE_PLATFORM") {
        return match forced.to_str().unwrap_or_default() {
            "macos" => Ok("macos"),
            "linux" => Ok("linux"),
...
pub fn collect_packages(context: &Context, groups: &[String]) -> Result<Vec<Package>, String> {
...
pub fn sorted_directories(directory: &Path) -> Result<Vec<PathBuf>, String> {
...
pub fn validate_relative(value: &str) -> Result<(), String> {
