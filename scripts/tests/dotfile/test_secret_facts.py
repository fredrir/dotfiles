...
from tools.dotfile.secret import facts
...
def test_render_leaves_ordinary_braces_alone():
    ...
    rendered, missing = facts.render(text, {})
    assert rendered == text
    assert missing == []


@needs_sops
def seed(secret, writer, body=FACTS):
    return secret("edit", "facts.enc.yaml", editor=writer(body))


@needs_sops
def test_a_template_renders_from_facts(vault, writer):
    root, home, _env, secret = vault
    (root / "shared" / "ssh").mkdir()
    (root / "shared" / "ssh" / ".secret").write_text("")
    (root / "targets").write_text("shared/ssh = ~/.ssh\n")
...
def test_the_ciphertext_keeps_the_keys_readable(vault, writer):
    ...
    assert seed(secret, writer).returncode == 0
    sealed = (root / "facts.enc.yaml").read_text()
    assert "hosts:" in sealed
    assert "203.0.113.77" not in sealed


@needs_sops
def test_an_unknown_fact_blocks_and_names_itself(vault, writer):
    root, _home, _env, secret = vault
    (root / "shared" / "ssh").mkdir()
    (root / "shared" / "ssh" / ".secret").write_text("")
    (root / "targets").write_text("shared/ssh = ~/.ssh\n")
...
@needs_sops
def test_fact_values_become_canaries(vault, writer, tool):
    root, _home, env, secret = vault
    seed(secret, writer)
...
@needs_sops
def test_facts_lists_names_and_never_values(vault, writer):
    root, _home, _env, secret = vault
    (root / "shared" / "ssh").mkdir()
    (root / "shared" / "ssh" / ".secret").write_text("")
    (root / "targets").write_text("shared/ssh = ~/.ssh\n")
...
@needs_sops
def test_facts_can_list_only_the_unreferenced(vault, writer):
    _root, _home, _env, secret = vault
    seed(secret, writer)
    result = secret("facts", "--unused")
    assert "hosts.parser.origin" in result.stdout
...
@needs_sops
def test_transcript_redaction_strips_fact_values(vault, writer):
