...
`dmux`, the wezterm-mux + tmux session manager, moved to its own repository:
https://github.com/fredrir/dmux. Its command reference and flags are
documented there now, along with the `dmx`/`ssa`/`ssm` shell wrappers.
