...
| `dmux doctor` | Probe the environment transport selection depends on, including where `DMUX_WEZ_FIRST` came from (process env, launchd/systemd session, per-host file) and whether it survives a reboot. `--format json` for the envelope (`result.wez_first`); deprecated `--json` for the bare probe object. |
