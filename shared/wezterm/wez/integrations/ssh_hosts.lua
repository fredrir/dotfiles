...
local function read_hosts(path, out, seen)
  ...
  for line in file:lines() do
    -- Only literal Host entries: patterns and Match blocks name no single host.
...
-- ~/.ssh/config and its config.d remain the single source of truth for hosts;
...
function M.apply(config)
  ...
  for _, host in ipairs(M.hosts()) do
    table.insert(menu, { label = 'ssh ' .. host, args = { 'ssh', host } })
  end
