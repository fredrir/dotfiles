...
local function clean_selection(window, pane)
    ...
    wezterm.background_child_process {'sh', '-c', 'clean-copy --stdin < "$1"; rm -f "$1"', 'sh', path}
end
...
function M.apply(config)
    ...
    table.insert(keys, {
        key = 'c',
        mods = 'CTRL|SHIFT',
        action = act.Multiple {act.CopyTo 'Clipboard', wezterm.action_callback(clean_selection)}
    })
    config.keys = keys
end
